...
5. **get_harvest_cycle_plants_summary** - Returns a lightweight plant list in a harvest cycle
...
7. **search_plant_harvest_summaries** - Searches grouped plant harvest summaries for the authenticated user
  ...
  - Returns: One row per plant, grow condition, and harvest cycle with earliest seeding, earliest transplant, earliest harvest, latest harvest, and distinct non-empty notes from `PlantHarvestCycle`.
